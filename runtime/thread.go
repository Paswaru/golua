...
func (t *Thread) end(args []Value, err error, exception interface{}) {
	...
	// control to the caller: sendResumeValues lets the caller run again, and
	// it shares this thread's runtimeContextManager, so releasing afterwards
	// would touch that manager from this goroutine while the caller is
	// already using it.
